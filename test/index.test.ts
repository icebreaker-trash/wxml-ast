...
describe('[Default]', () => {
  ...
  test('attr-value-with-gt-000 with render', async () => {
    const nodes = await parse('<view id=">"></view>')
    const str = render(nodes)
    expect(str).toBe('<view id=">"></view>')
  })
  ...
  test('attr-value-with-single-quote-000 with render', async () => {
    const nodes = await parse('<tag name="\'"></tag>')
    const str = render(nodes)
    expect(str).toBe('<tag name="\'"></tag>')
  })
  ...
  test('element-node-nested-000 with render', async () => {
    const nodes = await parse(
      redent(`<view class="view-class {{someClass}}">
    <text>{{a <= b ? c : d}}</text>
    <include src="../some-template.wxml" />
  ...
  test('element-node-nested-001 with render', async () => {
    const nodes = await parse(
      redent(`<navigator>
      <primary-button>立即体验</primary-button>
    </navigator>`)
  ...
  test('element-node-nested-003 with render', async () => {
    const nodes = await parse(
      redent(`<view class="view-class {{someClass}}">
    <text>{{a >= b ? c : d}}</text>
    <include src="../some-template.wxml" />
  ...
  test('element-node-self-closing-tag-000 with render', async () => {
    const nodes = await parse(redent('<tag/>'))
    const str = render(nodes)
    expect(str).toBe('<tag></tag>')
  })
  ...
  test('element-node-self-closing-tag-001 with render', async () => {
    const nodes = await parse(redent('<tag />'))
    const str = render(nodes)
    expect(str).toBe('<tag></tag>')
  })
  ...
  test('inline-wxs-module-000 with render', async () => {
    const nodes = await parse(
      redent(`<wxs module="status">
      function get(index, active) {
        if (index < active) {
  ...
  test('tag-name-with-space-before-000 with render', async () => {
    const nodes = await parse(redent('< tag></ tag>'))
    const str = render(nodes)
    expect(str).toBe('< tag>')
  })
  ...
  test('text-node-001 with render', async () => {
    const nodes = await parse(redent('<>'))
    const str = render(nodes)
    expect(str).toBe('<>')
  })
  ...
  test('text-note-with-mustache-000 with render', async () => {
    const nodes = await parse(
      redent(`<view class="view-class {{someClass}}">
    <text>{{a<=b?c:d}}</text>
    <include src="../some-template.wxml" />
  ...
  test('text-note-with-mustache-001 with render', async () => {
    const nodes = await parse(
      redent(`<view class='view-class {{someClass || "otherClass"}}'>
    <text>{{a<=b?c:d}}</text>
    <include src="../some-template.wxml" />
  ...
  test('text-note-with-mustache-001 with render 2', async () => {
    const nodes = await parse(
      redent(`<view class="view-class {{someClass || 'otherClass'}}">
    <text>{{a<=b?c:d}}</text>
    <include src="../some-template.wxml" />
